...
module.exports.nowPlaying = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      const { album, duration_ms, name, artists, external_urls } = item;
...
module.exports.recentlyPlayed = async () => {
  try {
    const access_token = await getAccessToken();
    const response = await axios(RECENTLY_PLAYED_ENDPOINT, {
      method: "GET",
      headers: { Authorization: access_token },
    });
    ...
    if (response.status === 200 && items.length) {
      const { album, duration_ms, name, artists, external_urls } =
        items[0].track;
...
async function imageToBase64(image_link) {
  try {
    const response = await axios(image_link, { responseType: "arraybuffer" });
