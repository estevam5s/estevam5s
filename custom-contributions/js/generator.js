class Generator {
  constructor() {
    ...
    for (let i = 0; i < this.days.length; i++) {
      this.days[i].addEventListener(
        "click",
        (event) => this.handleDayClick(event, i),
        false
      );
    ...
    this.resetActivityButton.addEventListener(
      "click",
      this.resetActivity,
      false
    );
  ...
  handleDayClick = (event, index) => {
    this.daysActivity[index] = this.daysActivity[index]
      ? this.daysActivity[index] + 1
      : 1;
